...
class LiveDirectory extends EventEmitter {
	 ...
	 * @param {String} options.paths Array of path of the desired directory
	...
	constructor(options) {
		...
		options.paths = options.paths.map(resolve_path);
	...
	_ignore_path(path, stats) {
		...
		if (this.#filters.ignore && this.#filters.ignore(path, stats))
			return true;
	...
	async _initiate_watcher() {
		...
		for (let i in paths) {
			...
			this.#watcher = chokidar.watch(path, {
				ignored: (path, stats) => {                    
                    return this._ignore_path(path, stats)
                },
				awaitWriteFinish: {
					pollInterval: 100,
	 ...
	 * @param {String} path
	 */
	_relative_path(path, idx) {
        // console.log({idx});
		return forward_slashes(path).replace(this.#options.paths[idx], '');
	}
	...
	ready() {
		...
		if (this.#ready_promise === undefined)
			this.#ready_promise = new Promise(
				(resolve) => (this.#ready_resolve = resolve)
			);
	...
	get(path) {
		...
		if (typeof path !== 'string')
			throw new Error(
				'LiveDirectory.get(path) -> path must be a String.'
			);
